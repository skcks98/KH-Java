...
public class StudentService extends Object {
	 ...
	 * 
	 * // testList에서 꺼내온 인덱스 요소가 Student혹은 상속 관계인지 검사 // 만약 Student 라면 이름은 꺼내오고싶다
	 * for(Object e : testList) {
	 * 
	 * if (e instanceof Student) { // 맞다면 접근하여 이름 출력
	...
	public String updateStudent() throws InputMismatchException {
		...
		if (studentList.isEmpty()) {
			...
			// 입력받은 index번째 수정할 학생정보를 세팅 -> 수정
			// index 번째에 있던 기존 학생 정보가 반환되고, 그 객체를 temp에 저장
	 ...
	 * name 값이 같은지 비교 - 일치하는 경우 Student 객체 출력 - 검색 결과 없습니다 출력
	 */
	...
	/**
	 * 이름에 특정 문자열이 포함되는 학생을 찾아서 조회하는 메서드 문자열을 입력받아 studentList에서 꺼내온 Student 객체의
	 * name 값에 포함되는 문자열인지 검사 - 포함되는 경우 Student 객체 출력 - 검색 결과가 없습니다 출력
	 */
