...
public class FileService {
	...
	public void method1() {
		...
		File directory = new File("/io_test/20240926");
	...
	public void method2() {
		...
		if(!file.exists()) { // 폴더가 존재하지 않으면
			...
			try {
				...
				if(file.createNewFile()) { // 파일을 생성했다, true인 경우
	...
	public void method3() {
		...
		for(File f : files) {
			...
			String result = String.format("%-20s %-20s %-5s %10s", fileName, date, type, size); // 20칸 확보하고 좌측정렬
	...
	/*
	 * 스트림은 파일이나 데이터의 입출력 작업을 위해서 사용돔
