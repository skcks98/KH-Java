...
public class ByteService {
	 ...
	 * 
	 * - 문자열, 이미지, 영상, 오디오, pdf 등 모든것을 입/출력 할 수 있음 (단, 통로가 좁다보니 속도가 좀 느림, char 형 문자는
	 * 깨질 우려가 있음)
	 * 
	 */

	// IO 관련된 코드 IOException이 발생될 가능성이 높다.
	// -> 거의 모든 메서드가 thros IOException
	...
	public void fileByteOutput() {
		...
		try {
			...
			sb.append("Hello woerld!!\n");
			sb.append("1234567890\n");
			sb.append("~!@#$$%%^&*()_+\n");
			sb.append("가나다라마바사아\n");
			sb.append("와 자바 너무 재밌어");
			sb.append("진짜로! 정말로! 화이팅");

			// 출력방법 1 : 한글자씩 (2byte)출력

			// 2-> byte 범주의 문자는 자이토 ㅣ시반으로 깨짐
			...
			System.out.println("수행시간] : " + (endTime - startTime) + "ns"); // 55500ns
	...
	public void bufferedFileByteOutput() {
		...
		/*
		 * 버퍼(Buffer) 란? - 데이터를 모아두는 박스, 바구니 같은 메모리 공간
		 * 
		 * 버퍼링 - 인터넷 속도가 낮음 + 스트리밍 코드가 효율이 안좋음 -> 동영상 재생 중에 미리 앞에 재생될 부분을 다운 받아놔서 영상을
		 * 끊김없이 볼수 있게함
		 * 
		 * -> 다운받아서 저장해놓은 공간 == 버퍼
		 * 
		 ...
		 * 
		 * - FileOutputStream은 1바이트씩 데이터를 입출력 -> 통로가 좁아서 데이터 입출력이 느림
		 * 
		 * -> 보조 스트림 이용 시 버퍼에 입출력할 내용을 모아서 한 번에 입출력 --> 기반 스트림을 이용하는 횟수가 적어짐 -> 성능, 시간
		 * 효율 up!
		 * 
		 */
		...
		try {
			...
			System.out.println("수행시간] : " + (endTime - startTime) + "ns"); // 22900ns

			bos.flush(); // 스트림 안에 남아있는 모든 데이터를 모두 밀어냄
			...
			try {
				...
				// 보조 스트림(bos) close()시
				// 보조 스트림 생성에 사용된 기반 스트림(fos)도 같이 close() 됨
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
	...
	public void fileByteInput() {
		...
		try {
			...
			while (true) {
				...
				sb.append((char) value); // char 강제 형변환 해서 글자 형태로 sb 추가
			}
	 ...
	 * 
	 * 
	 * InputStream의 경우 외부 파일을 읽어올 때 사용 -> 읽어오는 파일의 크기가 작으면 보조
	 * 
	 * 
	 */
	 ...
	 * 
	 * [실행화면] 파일 경로 입력 : /io_test/20240926/노래가사.txt 복사 완료 :
	 * /io_test/20240926/노래가사_copy.txt
	 * 
	 * 1) 입력된 경로에 파일이 있는지 검사
	 * 
	 * 2) 있으면 파일 내용을 모두 읽어오기 : FileInputStream + BufferedInputStream
	 * 
	 * 3) 읽어온 내용을 같은 경로 위치에 "파일명_copy" 이름으로 출력 : FileOutputStream +
	...
	public void fileCopy() {
		...
		// 스트림 참조 변수 선언
		BufferedReader br = null; // 문자열 입력용 스트
		...
		try {
			...
			int insertPoint = target.lastIndexOf("."); // 22번째 인덱스
