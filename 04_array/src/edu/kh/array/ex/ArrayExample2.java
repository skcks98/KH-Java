...
public class ArrayExample2 {
	...
	public void deepCopy() {
		...
		System.out.println(arr);  // 서로 다른 주소값을 참조하고 있다.
	...
	public void createLottoNumber() {
		...
		for(int i=0; i<lotto.length; i++) {
			...
			for(int x=0; x<i; x++) {
				...
				if(random == lotto[x]) {
					...
					// -> i값을 인위적으로 1 감소시켜서 난수 발생의 기회를 한번 더 주는것
					
					break;
					// 앞쪽에서 중복데이터를 발견하면
					// 남은 값을 비교할 필요 없다.
					// -> 효율 향상을 위해 중복 검사하는 for문 종료
				}
			}
		...
		// 정렬은 되어 있지 않지만, 중복 제거가 된 배열이 완성
		
		// 7) 오름차순 정렬
		// -> 선택, 삽입, 버블, 퀵 등등
		// Array.sort(배열명) : 배열 내 요소값들이 오름차순 정렬됨
